...
public class SQLFactory
{
	...
	public static SQLTicket selectData(String table, String selection, String[] conditions, String[] placeHolders,
			int page, int itemSize, SQLCallback callback)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT " + selection + " FROM " + table + " WHERE ");
		...
		int offset = (page - 1) * itemSize;

		sb.append(" LIMIT " + offset + "," + itemSize);
	...
	public static SQLTicket deleteData(String table, String condition, SQLCallback callback)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("DELETE FROM `" + table + "` WHERE " + condition);
		return new SQLTicket(new SQLQuery(sb.toString()), callback);
	}
}
